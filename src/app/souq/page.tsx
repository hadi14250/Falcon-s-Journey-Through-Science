      ...
      {avatars.map((a) => {
        ...
        return (
          <motion.button
            ...
            onClick={() => onPick(a.id)}
            whileTap={{ scale: 0.92 }}
            className={`relative shrink-0 w-10 h-10 rounded-xl overflow-hidden border-2 bg-[#FFF1DC] flex items-center justify-center ${
          ...
          >
            ...
            {isActual && (
              <span
                className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-[#009639] border-[1.5px] border-white"
      ...
      {enabledCompanions.map((c) => {
        ...
        return (
          <motion.button
            ...
            onClick={() => onPick(c.id)}
            whileTap={{ scale: 0.92 }}
            className={`relative shrink-0 w-10 h-10 rounded-xl overflow-hidden border-2 bg-gradient-to-b from-[#FFF7DC] to-[#FFE9A8] flex items-center justify-center ${
              isStaged ? "border-[#D4AF37]" : "border-[#E5D9B8]"
            }`}
            style={{
              boxShadow: isStaged
...
export default function SouqPage() {
  ...
  const handleBuy = () => {
    ...
    if (ok) {
      ...
      setTimeout(() => setPurchaseFlash(false), 500);
    ...
    <div className="relative h-dvh bg-[var(--color-lesson-bg)] flex flex-col overflow-hidden overflow-x-hidden">
      ...
      <div className="sticky top-0 z-30 bg-[var(--color-lesson-bg)]/95 backdrop-blur-sm px-4 pt-3 pb-2 border-b border-[#E5D9B8]">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          {/* Dirham balance pill — when balance is 0, expands into a tap-to-play
              hint that takes the player to the map to earn. */}
          {dirhams === 0 ? (
            <motion.button
              onClick={() => navigate("/map", { label: "Unrolling the Map", labelAr: "نفتح الخريطة" })}
              ...
              animate={purchaseFlash ? { scale: [1, 1.18, 1] } : undefined}
            ...
            <motion.div
              className="flex items-center gap-1.5 bg-white border-2 border-[#1A1A2E] rounded-full px-3 py-1.5"
              style={{ boxShadow: "0 3px 0 #C9B58A" }}
              animate={purchaseFlash ? { scale: [1, 1.18, 1] } : undefined}
              transition={{ duration: 0.4 }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/uae-dirham.webp" alt="" width={20} height={20} />
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center justify-start px-3 pt-3 pb-4 gap-2.5">
        {/* Character card — proper square hero. */}
        <div
          className="w-44 h-44 md:w-56 md:h-56 rounded-3xl bg-gradient-to-b from-[#FFF7DC] to-[#FFE9A8] border-2 border-[#1A1A2E] overflow-hidden relative"
      ...
      {/* === Item rail —
           ...
           Desktop (md+): all tiles laid out flat in one row, no scroll. */}
      <div className="px-3 pb-2 shrink-0 h-[156px] md:h-[186px]">
        ...
        <div className="max-w-2xl mx-auto">
          <div className="h-[58px] flex items-center">
            {previewItem ? (
              ...
              <div className="w-full text-center font-body text-sm text-[#1A1A2E]/55">
